...
def refactor_label_me_json_dict_so_unique_class_instance_id_verb_target_are_keys_and_contours_are_values(json_dict: dict, 
    ...
    for contour_info in json_dict['shapes']:
        ...
        verb_name  = contour_info['verb']
        target_name  = contour_info['target']
        
        instance_id =  contour_info['group_id']
        contour_pts = contour_info['points']
        
        if((class_name, instance_id, verb_name, target_name) in refactored_json_dict): 
            refactored_json_dict[(class_name, instance_id, verb_name, target_name)].append(contour_pts)    
        else:
            refactored_json_dict[(class_name, instance_id, verb_name, target_name)] =  [contour_pts] 
...
def get_list_of_contour_dicts_for_all_instances_in_img_from_json_relations(json_dict: dict, 
                                                                 vis: bool = False):
    ...
    INSTRUMENT_ID_TO_NAME_DICT = TripletSegmentationVariables.categories['instrument']
    INSTRUMENT_NAME_TO_ID_DICT = {instrument_name: instrument_id for instrument_id, instrument_name in INSTRUMENT_ID_TO_NAME_DICT.items()}
    
    VERB_ID_TO_NAME_DICT = TripletSegmentationVariables.categories['verb']
    VERB_NAME_TO_ID_DICT = {verb_name: verb_id for verb_id, verb_name in VERB_ID_TO_NAME_DICT.items()}
    
    TARGET_ID_TO_NAME_DICT = TripletSegmentationVariables.categories['target']
    TARGET_NAME_TO_ID_DICT = {target_name: target_id for target_id, target_name in TARGET_ID_TO_NAME_DICT.items()}
    
    refactored_json_dict = {}
    refactor_label_me_json_dict_so_unique_class_instance_id_verb_target_are_keys_and_contours_are_values(json_dict, 
                                               refactored_json_dict)
        
       
    list_of_contour_info_dicts_for_all_instances_in_img = [] 
    for instrument_name_instance_id_verb_name_target_name, contour_pts_list in refactored_json_dict.items():
        instrument_name = instrument_name_instance_id_verb_name_target_name[0]
        instance_id = instrument_name_instance_id_verb_name_target_name[1]
        verb_name = instrument_name_instance_id_verb_name_target_name[2]
        target_name = instrument_name_instance_id_verb_name_target_name[3]
        ...
        single_instance_contours_info_dict = {
            'instance_id':  instance_id,
            'instrument_id': INSTRUMENT_NAME_TO_ID_DICT[instrument_name],
            'verb_id': VERB_NAME_TO_ID_DICT[verb_name],
            'target_id': TARGET_NAME_TO_ID_DICT[target_name],
            'contours_for_an_instance_as_list': contour_points_for_an_instance_polygon_xy,
...
def contour_to_coco_relations(annotation_name: str, 
                    annotation_dir: str, 
                    image_id: int, 
    ...
    First Get label_me_json or npy to a unified format called contour_dict. it is a list of dicts that looks like this
     {
            'instance_id': id of instance in the image, but this is not stored in coco,
            'triplet_id': int -> triplet_id for referencing with coco,
            'contours_for_an_instance_as_list': list -> contours in the instance as a list,
            'area_for_instance':  float -> amount_of_pixels with 1 ,
            'bbox_for_instance':  list -> x_min, y_min, width, height
        }  
    ...
    if annotation_name.split('.')[-1] == 'json':   
        json_dict =  read_from_json(join(annotation_dir, annotation_name ))
        contour_dicts_for_all_instances_in_img = get_list_of_contour_dicts_for_all_instances_in_img_from_json_relations(json_dict, vis)  
    else: 
    ...
    for single_instance_contours_info_dict in contour_dicts_for_all_instances_in_img:
        ...
        instrument_id = int(single_instance_contours_info_dict['instrument_id']) 
        verb_id = int(single_instance_contours_info_dict['verb_id'])        
        target_id = int(single_instance_contours_info_dict['target_id'])                
...
def initalize_coco_annotation(instrument_id_to_class, 
                              verb_id_to_class,
                              target_id_to_class):
    coco_annotation = {
        "images": [],
    ...
    # instrument categories the default category.
    for instrument_class_id, instrument_class_name in instrument_id_to_class.items():
        instrument_category = {
            "id": int(instrument_class_id),
            "name": instrument_class_name,
            "supercategory": "instrument"
        }
        coco_annotation["categories"].append(instrument_category)
    
    # verb categories.
    for verb_class_id, verb_class_name in verb_id_to_class.items():
        verb_category = {
            "id": int(verb_class_id),
            "name": verb_class_name,
            "supercategory": "verb"
        }
        coco_annotation["verb_categories"].append(verb_category)
        
     # target categories.
    for target_class_id, target_class_name in target_id_to_class.items():
        target_category = {
            "id": int(target_class_id),
            "name": target_class_name,
            "supercategory": "target"
        }
        coco_annotation["target_categories"].append(target_category)    

    return coco_annotation        
        
def annotations_to_coco_relations(dataset_path: str,
                        output_json_path: str,
                        vis: bool = False):
    """generate the coco_format for the relations format. Which has a separate target and verb information.  
    ...
    coco_annotation = initalize_coco_annotation(instrument_id_to_instrument_class_dict,
                                                verb_id_to_instrument_class_dict,
                                                target_id_to_instrument_class_dict)
    ...
    for i, filename in enumerate(img_list):          
        ...
        contour_to_coco_relations(mask_name, mask_dir, image_id, coco_annotation=coco_annotation, vis=vis)      
