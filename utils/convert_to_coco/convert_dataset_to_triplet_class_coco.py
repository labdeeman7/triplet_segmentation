...
def get_list_of_contour_dicts_for_all_instances_in_img_from_json(json_dict: dict, 
    ...
    the dicts, 
        {
            'instance_id': id of instance in the image, but this is not stored in coco,
            'triplet_id': int -> triplet_id which is the class_id in coco format,
            'contours_for_an_instance_as_list': list -> contours in the instance as a list,
            'area_for_instance':  float -> amount_of_pixels with 1 ,
            'bbox_for_instance':  list -> x_min, y_min, width, height 
    ...
    TRIPLET_ID_TO_NAME_DICT = TripletSegmentationVariables.categories['triplet']
    TRIPLET_NAME_TO_ID_DICT = {triplet_name: triplet_id for triplet_id, triplet_name in TRIPLET_ID_TO_NAME_DICT.items()}
    ...
    list_of_contour_info_dicts_for_all_instances_in_img = [] 
    for class_name_instance_id_verb_name_target_name, contour_pts_list in refactored_json_dict.items():
        class_name = class_name_instance_id_verb_name_target_name[0]
        instance_id = class_name_instance_id_verb_name_target_name[1]
        verb_name = class_name_instance_id_verb_name_target_name[2]
        target_name = class_name_instance_id_verb_name_target_name[3]
        
        triplet_name = f'{class_name},{verb_name},{target_name}'
        ...
        single_instance_contours_info_dict = {
            'instance_id':  instance_id,
            'triplet_id': TRIPLET_NAME_TO_ID_DICT[triplet_name],
            'contours_for_an_instance_as_list': contour_points_for_an_instance_polygon_xy,
...
def contour_to_coco_triplet(annotation_name: str, 
                    annotation_dir: str, 
                    image_id: int, 
    ...
    for single_instance_contours_info_dict in contour_dicts_for_all_instances_in_img:
        ...
        triplet_id = int(single_instance_contours_info_dict['triplet_id'])         
...
def initalize_coco_annotation(id_to_class):
    ...
    for triplet_id, triplet_name in id_to_class.items():
        category = {
            "id": int(triplet_id),
            "name": triplet_name,
...
def annotations_to_coco_triplet(dataset_path: str,
    ...
    triplet_id_to_triplet_class_dict = TripletSegmentationVariables.categories['triplet']
    coco_annotation = initalize_coco_annotation(triplet_id_to_triplet_class_dict)
    ...
    for i, filename in enumerate(img_list):          
        ...
        contour_to_coco_triplet(mask_name, mask_dir, image_id, coco_annotation=coco_annotation, vis=vis)      
