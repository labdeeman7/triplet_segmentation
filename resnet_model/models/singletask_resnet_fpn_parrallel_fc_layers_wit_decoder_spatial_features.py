...
class SingleTaskResNetFPNWithParallelTransformersSpatialFeatures(nn.Module):
    ...
    def forward(self, img, mask, instrument_id):
        ...
        instrument_feat_pooled = nn.AdaptiveAvgPool2d((16, 16))(instrument_feat)  # (B, C, 16, 16)
        background_feat_pooled = nn.AdaptiveAvgPool2d((16, 168))(background_feat)  # (B, C, 16, 16)
        
        # Get B, C
        B, C, H, W = instrument_feat_pooled.shape  # H=W=16 here
        
        # Flatten into tokens
        instrument_tokens = instrument_feat_pooled.view(B, C, -1).permute(2, 0, 1)   # (64, B, C)
        background_tokens = background_feat_pooled.view(B, C, -1).permute(2, 0, 1)  # (64, B, C)
        
        # Normalize
        instrument_query = instrument_tokens / (instrument_tokens.norm(dim=-1, keepdim=True) + 1e-6)
        background_memory = background_tokens / (background_tokens.norm(dim=-1, keepdim=True) + 1e-6)
        
        # Add instrument class embedding to all tokens in the query (broadcasted)
        class_embed = self.instrument_class_embedding(instrument_id)  # (B, embed_dim)
        class_embed = class_embed.unsqueeze(0).expand(instrument_query.size(0), -1, -1)  # (64, B, embed_dim)
        
        # Concatenate class embedding to each spatial token before passing to the decoder
        instrument_query_with_class = torch.cat((instrument_query, class_embed), dim=-1)  # (64, B, C + embed_dim)
        
        # Project into decoder space
        instrument_embed = self.instrument_embedding(instrument_query_with_class)  # (64, B, decoder_hidden_dim)
        background_embed = self.background_image_embedding(background_memory)      # (64, B, decoder_hidden_dim)
        
        # Final normalization
        instrument_embed = instrument_embed / (instrument_embed.norm(dim=-1, keepdim=True) + 1e-6)
        background_memory = background_embed / (background_memory.norm(dim=-1, keepdim=True) + 1e-6)
        ...
        for i in range(img.size(0)):
            ...
            if instr_id in self.transformer_decoders:
                ...
                pooled_output = decoder_output.mean(dim=0)  # [B, decoder_hidden_dim]
        
                # Pass through the corresponding fully connected layer
                task_pred = self.fc_task_dict[instr_id](pooled_output)  # [B, num_local_classes]
                task_pred = self.pad_tensor(task_pred, max_output_size)  # [B, max_output_size]
