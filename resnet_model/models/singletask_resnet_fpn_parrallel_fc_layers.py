...
# Multitask ResNet
class SingleTaskResNetFPNForParallelFCLayers(nn.Module):
    def __init__(self, num_instruments, instrument_to_task_classes):
        
        super(SingleTaskResNetFPNForParallelFCLayers, self).__init__()
        ...
        self.instrument_to_task_classes = instrument_to_task_classes


        # Fully connected layers for task
    ...
    def forward(self, img, mask, instrument_id):
       # Concatenate the image and mask along the channel dimension
        ...
        # Find max number of output classes
        max_output_size = max(fc[-1].out_features for fc in self.fc_task_dict.values()) 
        # print(max_output_size)     

        # Process each sample separately to select the correct FC layer
        batch_preds = []
        for i in range(img.size(0)):
            instr_id = str(instrument_id[i].item())  # Convert to string for key lookup
            if instr_id in self.fc_task_dict:
                task_pred = self.fc_task_dict[instr_id](combined_features[i].unsqueeze(0))
                # print('task_pred before', task_pred)
                task_pred = self.pad_tensor(task_pred, max_output_size)  # Pad to max size
                # print('task_pred after', task_pred)
            else:
                raise ValueError(f"Instrument ID {instr_id} not found in fc_task_dict.")
